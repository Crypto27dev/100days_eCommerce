...
function Dashboard() {
    ...
    const lineState = {
        labels: orderAmountList.slice(orderAmountList.length > 5 ? Math.max(orderAmountList.length - 5, 0) : 0).map((od, i) => `Order ${i + 1}`),
        datasets: [
            {
                label: "Total Amount",
                backgroundColor: ["#4CAF50"],
                hoverBackgroundColor: ["#4caf4fb6"],
                data: orderAmountList.slice(orderAmountList.length > 5 ? Math.max(orderAmountList.length - 5, 0) : 0).reverse(),
            },
        ],
    };
